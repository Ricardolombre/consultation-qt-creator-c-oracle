...
AjouterRDV::AjouterRDV(DatabaseManager *_db, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AjouterRDV)
{
    ...
    QStringList nomMedecin = getNamesFromQuery("SELECT NOM||' '||PRENOM as NOMS FROM T_MEDECIN", "NOMS");

    model = new QStringListModel(nomMedecin, this);
...
void AjouterRDV::on_btnAjoutRDV_clicked()
{
    ...
    if(dbManager->openDatabase())
    {
        ...
        query = QString("SELECT ID_MEDECIN FROM T_MEDECIN WHERE NOM||' '||PRENOM = '"+ nomM +"'");
        Qquery = dbManager->executeQuery(query);
        if(Qquery.isActive()){
            while(Qquery.next()){
                id_medecin = Qquery.value("ID_MEDECIN").toInt();
            }
        }
    }

    if(id_medecin == -1 || id_patient == -1){
        QMessageBox::warning(this, "Rendez-vous", "Les informations du patient ou du medecin sont erronees, veuillez recommencer");
        return;
    }

    QString date = "TO_DATE('"+ dateRDV.toString("yyyy-MM-dd") + " " + heureRDV.toString("hh:mm:ss") +"', 'YYYY-MM-DD HH24:MI:SS')";

    QString query = QString("SELECT ID_PATIENT FROM T_RDV WHERE ID_PATIENT = "+ QString::number(id_patient) +" AND DATERDV = "+ date);
    QSqlQuery Qquery = dbManager->executeQuery(query);
    if(Qquery.next())
    {
    ...
    query = QString("SELECT ID_MEDECIN FROM T_RDV WHERE ID_MEDECIN = "+ QString::number(id_medecin) +" AND DATERDV = "+ date);
    Qquery = dbManager->executeQuery(query);
    if(Qquery.next())
    {
        QMessageBox::warning(this, "Rendez-vous", "Le medecin a déjà un rendez-vous prévu pour cette date");
        return;
    }

    RDV rdv(dateRDV, heureRDV, description, id_medecin, id_patient);
